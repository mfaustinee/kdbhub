...
export const CommentsAndCorrectiveActionsComponent: React.FC<CommentsAndCorrectiveActionsComponentProps> = ({
  ...
  const hasCorrectiveActions = (recommendedActions || '').trim().length > 0;
        ...
        <div className="pt-4 border-t border-slate-100 grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="space-y-1.5">
            ...
            <input
              ...
              disabled={readOnly || !hasCorrectiveActions}
              className={`w-full px-3.5 py-2.5 rounded-xl border text-xs font-mono transition-colors outline-none ${
                !hasCorrectiveActions || readOnly
                  ? 'bg-slate-100/80 border-slate-200 text-slate-400 cursor-not-allowed select-none'
                  : 'bg-white border-slate-200 text-slate-800 focus:border-blue-500'
              }`}
            />
            {!hasCorrectiveActions && (
              <p className="text-[10px] text-slate-400 font-medium">Disabled: Enter corrective actions above to set a resolution date.</p>
            )}
          </div>
          ...
          <div className="space-y-1.5">
            <label className="text-[11px] font-bold text-slate-700 uppercase tracking-wider flex items-center gap-1.5">
              ...
              {hasCorrectiveActions && <span className="text-red-500 font-bold ml-0.5">*</span>}
            </label>
            <input
              type="text"
              required={hasCorrectiveActions}
              value={actionOwner}
              onChange={(e) => onChange({ actionOwner: e.target.value })}
              disabled={readOnly || !hasCorrectiveActions}
              placeholder={hasCorrectiveActions ? "e.g. Managing Director / Proprietor" : "Disabled: No corrective actions entered"}
              className={`w-full px-3.5 py-2.5 rounded-xl border text-xs transition-colors outline-none ${
                !hasCorrectiveActions || readOnly
                  ? 'bg-slate-100/80 border-slate-200 text-slate-400 cursor-not-allowed select-none'
                  : !actionOwner?.trim()
                    ? 'border-amber-300 focus:border-red-500 bg-amber-50/20 text-slate-800'
                    : 'border-slate-200 focus:border-blue-500 bg-white text-slate-800'
              }`}
              id="comments-action-owner-input"
            />
            {!hasCorrectiveActions ? (
              <p className="text-[10px] text-slate-400 font-medium">Disabled: Enter corrective actions above to assign a responsible person.</p>
            ) : !actionOwner?.trim() ? (
